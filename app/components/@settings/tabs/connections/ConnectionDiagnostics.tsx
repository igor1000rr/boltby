...
export default function ConnectionDiagnostics() {
  ...
  const runDiagnostics = async () => {
    try {
      ...
      const localStorageChecks = {
        ...
        appwriteConnection: localStorage.getItem('appwrite_endpoint'),
      };
      ...
      try {
        const pbResp = await fetch(`${appwriteEndpoint}/v1/health`);
        appwriteCheck = {
          ok: pbResp.ok,
          status: pbResp.status,
          message: pbResp.ok ? 'Appwrite is running' : 'Appwrite not responding',
        };
      } catch {
        appwriteCheck = { ok: false, message: 'Appwrite not reachable at ' + appwriteEndpoint };
      }
      ...
      if (appwriteCheck && !appwriteCheck.ok) {
        toast.warning('Appwrite is not running. Start it with: appwrite serve');
      }
  ...
  const resetAppwriteConnection = () => {
    try {
      localStorage.removeItem('appwrite_endpoint');
      toast.success('Appwrite URL reset to default (localhost:8090).');
      setDiagnosticResults(null);
    } catch (error) {
      log.error('Error clearing Appwrite data:', error);
      toast.error('Failed to reset Appwrite connection');
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            <>
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={classNames(
                    'text-xl font-semibold',
                    diagnosticResults.apiEndpoints.appwrite?.ok
                      ? 'text-green-500 dark:text-green-400'
                      : 'text-red-500 dark:text-red-400',
                  )}
                >
                  {diagnosticResults.apiEndpoints.appwrite?.ok ? 'Running' : 'Not Running'}
                </span>
              </div>
              <div className="text-xs text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary mt-2 flex items-center gap-1.5 truncate">
                ...
                URL: {diagnosticResults.localStorage.appwriteEndpoint || 'Not configured'}
              </div>
              <div className="text-xs text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary mt-2 flex items-center gap-1.5">
                <div className="i-ph:check-circle w-3.5 h-3.5 text-bolt-elements-item-contentAccent dark:text-bolt-elements-item-contentAccent" />
                Health:{' '}
                <Badge
                  variant={diagnosticResults.apiEndpoints.appwrite?.ok ? 'default' : 'destructive'}
                  className="ml-1"
                >
                  {diagnosticResults.apiEndpoints.appwrite?.ok ? 'OK' : 'Offline'}
                </Badge>
              </div>
            </>
          ) : (
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Button
          onClick={resetAppwriteConnection}
          disabled={isRunning}
          variant="outline"
          className="flex items-center gap-2 hover:bg-bolt-elements-item-backgroundActive/10 hover:text-bolt-elements-textPrimary dark:hover:bg-bolt-elements-item-backgroundActive/10 dark:hover:text-bolt-elements-textPrimary transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
