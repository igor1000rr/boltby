...
const log = createScopedLogger('ImportExportService');
...
export class ImportExportService {
  ...
  static async exportAllChats(db: IDBDatabase): Promise<{ chats: any[]; exportDate: string }> {
    ...
    try {
      ...
      log.info(`Successfully prepared ${sanitizedChats.length} chats for export`);
  ...
  static async resetAllSettings(db: IDBDatabase): Promise<void> {
    ...
    allLocalStorageKeys.forEach((key) => {
      if (!localStorageKeysToPreserve.includes(key)) {
        try {
          ...
          log.error(`Error removing localStorage item ${key}:`, err);
        }
      }
    });
    ...
    cookieKeys.forEach((key) => {
      if (!cookiesToPreserve.includes(key)) {
        try {
          ...
          log.error(`Error removing cookie ${key}:`, err);
        }
      }
    });
    ...
    if (!db) {
      log.warn('Database not initialized, skipping IndexedDB reset');
    } else {
    ...
    snapshotKeys.forEach((key) => {
      try {
        ...
        log.error(`Error removing snapshot ${key}:`, err);
      }
    });
  }
      ...
      Object.entries(data.core).forEach(([key, value]) => {
        if (value !== null && value !== undefined) {
          try {
            ...
            log.error(`Error importing core setting ${key}:`, err);
          }
        }
      });
    }
    ...
    if (data.providers) {
      ...
      if (data.providers.provider_settings) {
        try {
          ...
          log.error('Error importing provider settings:', err);
        }
      }
      ...
      providerCookies.forEach((key) => {
        if (data.providers[key]) {
          try {
            ...
            log.error(`Error importing provider cookie ${key}:`, err);
          }
        }
      });
    }
      ...
      Object.entries(data.features).forEach(([key, value]) => {
        if (value !== null && value !== undefined) {
          try {
            ...
            log.error(`Error importing feature setting ${key}:`, err);
          }
        }
      });
    }
    ...
    if (data.ui) {
      ...
      if (data.ui.bolt_tab_configuration) {
        try {
          ...
          log.error('Error importing tab configuration:', err);
        }
      }
      ...
      if (data.ui.promptId) {
        try {
          ...
          log.error('Error importing prompt ID:', err);
        }
      }
      ...
      uiCookies.forEach((key) => {
        if (data.ui[key]) {
          try {
            ...
            log.error(`Error importing UI cookie ${key}:`, err);
          }
        }
      });
    }
    ...
    if (data.connections) {
      ...
      if (data.connections.netlify_connection) {
        try {
          ...
          log.error('Error importing Netlify connection:', err);
        }
      }
      ...
      Object.entries(data.connections).forEach(([key, value]) => {
        if (key.startsWith('github_') && value !== null && value !== undefined) {
          try {
            ...
            log.error(`Error importing GitHub connection ${key}:`, err);
          }
        }
      });
    }
      ...
      debugLocalStorageKeys.forEach((key) => {
        if (data.debug[key] !== null && data.debug[key] !== undefined) {
          try {
            ...
            log.error(`Error importing debug setting ${key}:`, err);
          }
        }
      });
      ...
      debugCookies.forEach((key) => {
        if (data.debug[key]) {
          try {
            ...
            log.error(`Error importing debug cookie ${key}:`, err);
          }
        }
      });
    }
    ...
    if (data.updates) {
      if (data.updates.update_settings) {
        try {
          ...
          log.error('Error importing update settings:', err);
        }
      }
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          ...
          log.error('Error importing last acknowledged update:', err);
        }
      }
    }
      ...
      Object.entries(data.chatSnapshots).forEach(([key, value]) => {
        if (value !== null && value !== undefined) {
          try {
            ...
            log.error(`Error importing chat snapshot ${key}:`, err);
          }
        }
      });
    }
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (value !== null && value !== undefined) {
        ...
        try {
          ...
          log.error(`Error importing legacy setting ${key}:`, err);
        }
      }
    });
  }
  ...
  private static _safeGetItem(key: string): any {
    try {
      ...
      log.error(`Error getting localStorage item ${key}:`, err);
      return null;
    }
  }
  ...
  private static _getAllLocalStorage(): Record<string, any> {
    ...
    try {
      ...
      log.error('Error getting all localStorage items:', err);
    }
  ...
  private static _getGitHubConnections(_cookies: Record<string, string>): Record<string, any> {
    ...
    localStorageKeys.forEach((key) => {
      try {
        ...
        log.error(`Error getting GitHub connection ${key}:`, err);
        result[key] = null;
      }
    });
  ...
  private static _getChatSnapshots(): Record<string, any> {
    ...
    snapshotKeys.forEach((key) => {
      try {
        ...
        log.error(`Error getting chat snapshot ${key}:`, err);
        result[key] = null;
      }
    });
  ...
  private static _safeSetItem(key: string, value: any): void {
    try {
      ...
      log.error(`Error setting localStorage item ${key}:`, err);
    }
  }
  ...
  private static _safeSetCookie(key: string, value: any): void {
    try {
      ...
      log.error(`Error setting cookie ${key}:`, err);
    }
  }
}
