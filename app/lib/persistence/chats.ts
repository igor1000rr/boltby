...
const log = createScopedLogger('chats');
...
export async function getAllChats(db: IDBDatabase): Promise<Chat[]> {
  log.debug(`getAllChats: Using database '${db.name}', version ${db.version}`);
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      request.onsuccess = () => {
        ...
        log.debug(`getAllChats: Found ${result.length} chats in database '${db.name}'`);
        resolve(result);
      };

      request.onerror = () => {
        log.error(`getAllChats: Error querying database '${db.name}':`, request.error);
        reject(request.error);
      };
    } catch (err) {
      log.error(`getAllChats: Error creating transaction on database '${db.name}':`, err);
      reject(err);
    }
  });
}
