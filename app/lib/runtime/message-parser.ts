import type { ActionType, BoltAction, BoltActionData, FileAction, AppwriteBackendAction } from '~/types/actions';
...
export class StreamingMessageParser {
  ...
  #parseActionTag(input: string, actionOpenIndex: number, actionEndIndex: number) {
    ...
    if (actionType === 'appwrite') {
      ...
      (actionAttributes as AppwriteBackendAction).operation = operation as 'collection' | 'query';

      if (operation === 'collection') {
        const filePath = this.#extractAttribute(actionTag, 'filePath');

        if (filePath) {
          (actionAttributes as AppwriteBackendAction).filePath = filePath;
        }
      }
