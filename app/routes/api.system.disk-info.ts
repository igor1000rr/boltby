...
const log = createScopedLogger('api.system.disk-info');
  ...
  try {
    ...
    if (platform === 'darwin') {
      ...
      try {
        ...
        log.error('Failed to get macOS disk info:', error);
        return [
          {
            filesystem: 'Unknown',
            size: 0,
      ...
      try {
        ...
        log.error('Failed to get Linux disk info:', error);
        return [
          {
            filesystem: 'Unknown',
            size: 0,
      ...
      try {
        ...
        log.error('Failed to get Windows disk info:', error);
        return [
          {
            filesystem: 'Unknown',
            size: 0,
    ...
    log.error('Failed to get disk info:', error);
    return [
      {
        filesystem: 'Unknown',
        size: 0,
